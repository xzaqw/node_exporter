...
func NewGZZpoolListExporter(logger log.Logger) (Collector, error) {
	...
	return &GZZpoolListCollector{
		...
		gzZpoolListGuid: prometheus.NewGaugeVec(prometheus.GaugeOpts{
			Name: "zpool_guid",
			Help: "zpool guid.",
		}, metric.NewLabelNames("zpool", "guid")),
		...
		gzZpoolListCapacityAlloc: prometheus.NewGaugeVec(prometheus.GaugeOpts{
			...
			Help: "zpool iostat .",
		}, metric.NewLabelNames("pool", "vdev")),
		...
		}, metric.NewLabelNames("req_size", "pool", "timestamp")),
		...
		}, metric.NewLabelNames("pool", "timestamp")),
...
func (e *GZZpoolListCollector) parseZpoolGetPropsOutput(out string) error {
	...
	for _, line := range outlines[0 : l-1] {
		...
		case "size":
			...
			e.gzZpoolListSize.With(
				metric.NewLabels(map[string]string{"zpool": pool_name}),
			).Set(pval / 1024 / 1024)
		case "free":
			pval, err := strconv.ParseFloat(val, 64)
			if err != nil {
		...
		case "freeing":
			...
			e.gzZpoolListFreeing.With(
				metric.NewLabels(map[string]string{"zpool": pool_name}),
			).Set(pval)
		case "health":
			if (strings.Contains(val, "ONLINE")) == true {
				e.gzZpoolListHealth.With(
					metric.NewLabels(map[string]string{"zpool": pool_name}),
				).Set(1)
			} else {
				e.gzZpoolListHealth.With(
		...
		case "guid":
			e.gzZpoolListGuid.With(
				metric.NewLabels(map[string]string{"zpool": pool_name, "guid": val}),
			).Set(1)
		}
	}
	return nil
...
func (e *GZZpoolListCollector) parseZfsGetOutput(out string) error {
	...
	for _, line := range outlines[0 : l-1] {
		...
		e.gzZfsLogicalUsed.With(
			metric.NewLabels(map[string]string{"zpool": name}),
		).Set(float64(pval / 1024 / 1024))
	}
	return nil
}
...
func (e *GZZpoolListCollector) helperZpoolWithSet(pool string, vdev string,
	...
	vec.With(
		metric.NewLabels(map[string]string{"pool": pool, "vdev": vdev}),
	).Set(pval)
	return nil
}
...
func (e *GZZpoolListCollector) parseZpoolIostatRequestSizesOutput(out string, timestamp int64) error {
	...
	for _, table := range tables {
		...
		for _, line := range outlines[2:l] {
			...
			).Set(float64(ctr_async_write))
			if err != nil {
				return err
			}
		}
		e.gzZpoolIostatReadTotal.With(
			metric.NewLabels(map[string]string{
				"pool":      pool,
				"timestamp": strconv.FormatInt(timestamp, 10),
			}),
		).Set(float64(bytes_read_total))
		e.gzZpoolIostatWriteTotal.With(
			metric.NewLabels(map[string]string{
				"pool":      pool,
				"timestamp": strconv.FormatInt(timestamp, 10),
			}),
		).Set(float64(bytes_write_total))
